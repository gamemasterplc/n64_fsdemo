...
INCLUDE_DIRS += /usr/include/n64 /usr/include/n64/nusys audio_data include $(BUILD_DIR) $(BUILD_DIR)/include src asm .

C_DEFINES := $(foreach d,$(DEFINES),-D$(d))
DEF_INC_CFLAGS := $(foreach i,$(INCLUDE_DIRS),-I$(i)) $(C_DEFINES)

CFLAGS = -Werror=implicit-function-declaration -mdivide-breaks -G 0 -Os -g -mabi=32 -ffreestanding -mfix4300 $(DEF_INC_CFLAGS)
...
$(BUILD_DIR)/asm/bin_data.o: $(FILESYSTEM)
...
$(ELF): $(O_FILES) $(BUILD_DIR)/$(LD_SCRIPT)
	...
	$(V)$(LD) -L $(BUILD_DIR) -T $(BUILD_DIR)/$(LD_SCRIPT) -Map $(BUILD_DIR)/$(TARGET_STRING).map --no-check-sections -o $@ $(O_FILES) -L/usr/lib/n64/nusys -lnusys -lnualstl -L/usr/lib/n64 -lmus -lultra_rom -L$(N64_LIBGCCDIR) -lgcc
